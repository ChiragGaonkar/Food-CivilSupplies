...
session_start();
if (isset($_POST['aforgot_submit'])) {
    $aid = mysqli_escape_string($connection, $_POST['AID']);
    $aEmail = mysqli_escape_string($connection, $_POST['AEMAIL']);
    $newPassword = mysqli_escape_string($connection, md5($_POST['APASSWORD']));
    $confirmPassword = mysqli_escape_string($connection, md5($_POST['ACPASSWORD']));
    if ($newPassword === $confirmPassword) {
        $sql = "SELECT * FROM admin_data WHERE AEMAIL = '$aEmail' AND AID = $aid";
        $result = mysqli_query($connection, $sql);
        if (mysqli_num_rows($result) > 0) {
            $_SESSION['faid'] = $aid;
            $_SESSION['faEmail'] = $aEmail;
            ...
            $alname = $row['ALNAME'];
            $reason = 'forgot';
            sendUserOtp($aEmail, $_SESSION['fotp'], $fname, $alname, $reason);
            header('location:aotpforgot.php');
        } else {
...
<body>
    ...
    <div class="container-fluid" style="background-color: #8AA899; max-width: 100%; height:auto;">
        <div class="row justify-content-center align-items-center">
            <img src="../Images/AdminForgot.svg" class="col-md-4 img-fluid" alt="" style="margin: 100px 0px 0px 0px;">
            <form name="myform" action="" method="POST" class="col-md-4 myform" style="margin: 30px 0px 0px 0px;"
                ...
                <div class="form-group row">
                    <div class="input-group inputs" style="width: auto;">
                        <span class="input-group-text" style="width: 180px;">Admin ID</span>
                ...
                <div class="d-grid gap-2 inputs">
                    <input class="btn btn-success" type="submit" name="aforgot_submit" value="Get OTP"
                        style="background-color: #F2D349; border:#F2D349">
                </div>
                <h6 class="text-center inputs">or</h6>
                <h6 class="text-center inputs"><a href="alogin.php"
